...
export class MaterialvalidComponent implements OnInit {
    ...
    ngOnInit() {
        this._Bimservice.materialvaliddetails().subscribe((materialdata) => this.materialvalid = materialdata); //subscribing to the Observable returned by angular Bimservice
    }
    ...
    getifctypewall(): number {
        return this.materialvalid.filter(materialvalid => materialvalid.IfcType === "IFCWall").length;

    }

    getifctypecolumn(): number {
        return this.materialvalid.filter(materialvalid => materialvalid.IfcType === "IFCColumn").length;

    }

    getifctypebeam(): number {
        return this.materialvalid.filter(materialvalid => materialvalid.IfcType === "IFCBeam").length;
